...
async function configureSensor() {
  try {
    ...
    // Set output rate to 100Hz (register 0x03, value 0x06 = 10Hz)
    await sendCommand(Buffer.from([0xff, 0xaa, 0x03, 0x06, 0x00]));
    ...
    // Set baud rate to 115200 (register 0x04, value 0x02 = 9600)
    await sendCommand(Buffer.from([0xff, 0xaa, 0x04, 0x02, 0x00]));
